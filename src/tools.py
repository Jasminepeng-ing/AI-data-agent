...
def generate_report(
    ...
    def _llm_report() -> str:
        ...
        user_q_block = (
            f"⚠️【用户完整原始问题——报告必须逐一回答所有子问题，不可遗漏任何一部分】：\n"
            f"「{user_question}」\n\n"
        ...
        prompt = (
            ...
            + user_q_block
            + "⚠️ 核心规则（按优先级）：\n"
            "1. 【全面覆盖】用户问题若包含多个子问题，报告必须逐一覆盖全部子问题，"
            "不得断章取义或以偏概全；以上方「用户完整原始问题」为最终判断依据；\n"
            "2. 【数据保真】报告所有数字必须严格来自下方原始查询数据，禁止推算或编造；\n"
            "3. 【内容充实】下方「对话摘要」包含 Agent 的完整分析洞察，"
            "报告须深度整合这些洞察（包括原因分析、特殊规律、业务解读），"
            "不要简单重复数字，要提炼 Agent 的解读结论；\n"
            "4. 【避免重复】同一信息只出现一次，表格展示数据则文字段落聚焦解读而非罗列数字；\n"
            "5. 【聚焦主题】严格围绕用户完整问题作答，不引用问题范围之外的数据或结论。\n\n"
